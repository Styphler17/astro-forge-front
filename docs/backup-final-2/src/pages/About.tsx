  ...
  return (
    <Layout>
              ...
              {aboutSettings.stats.map((stat, index) => {
                ...
                return (
                  <div key={index} className="text-center group">
                    <div className="bg-white dark:bg-astro-blue rounded-full w-16 h-16 flex items-center justify-center mx-auto mb-4 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-110 hover:-translate-y-1 group-hover:rotate-3">
                      <IconComponent className="h-8 w-8 text-astro-blue dark:text-white transition-colors duration-300 group-hover:text-astro-gold" />
                    </div>
                    <div className="bg-white/80 dark:bg-astro-blue/80 backdrop-blur-sm rounded-lg p-4 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 border border-white/20 dark:border-astro-blue/20">
                      <div className="text-3xl font-bold text-astro-blue dark:text-white mb-2 group-hover:text-astro-gold transition-colors duration-300">
                        {stat.value}
                      </div>
      ...
      {aboutSettings.journey_enabled && aboutSettings.timeline.length > 0 && (
        <section className="py-20 bg-white dark:bg-astro-blue">
          <div className="container mx-auto px-4">
            ...
            <div className="max-w-5xl mx-auto">
              <div className="relative">
                {/* Timeline line */}
                <div className="absolute left-8 top-0 bottom-0 w-0.5 bg-astro-blue dark:bg-white transform translate-x-1/2"></div>
                
                {aboutSettings.timeline.map((item, index) => (
                  <div key={index} className="relative mb-12">
                    <div className="flex items-center">
                      {/* Timeline dot */}
                      <div className="absolute left-8 transform -translate-x-1/2 w-4 h-4 bg-astro-blue dark:bg-white rounded-full border-4 border-white dark:border-astro-blue shadow-lg z-10"></div>
                      
                      {/* Content */}
                      <div className={`ml-16 flex-1 ${index % 2 === 0 ? 'text-left' : 'text-right'}`}>
                        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 border-l-4 border-astro-blue dark:border-white">
                          <div className="flex items-center justify-between mb-3">
                            <span className="text-2xl font-bold text-astro-blue dark:text-white">
                              {item.year}
                            </span>
                            <div className="w-8 h-8 bg-astro-blue dark:bg-white rounded-full flex items-center justify-center">
                              <div className="w-3 h-3 bg-white dark:bg-astro-blue rounded-full"></div>
                            </div>
                          </div>
