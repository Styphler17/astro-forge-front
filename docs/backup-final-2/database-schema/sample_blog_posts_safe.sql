-- Sample Blog Posts for Astro Forge Holdings (Safe Update Mode Compatible)
-- This file contains sample blog posts to populate the blog_posts table

-- Clear existing blog posts (safe update mode compatible)
-- First, let's get the IDs of existing posts and delete them specifically
SET @existing_ids = (SELECT GROUP_CONCAT(id) FROM blog_posts);
SET @delete_sql = CONCAT('DELETE FROM blog_posts WHERE id IN (', IFNULL(@existing_ids, ''), ')');
PREPARE stmt FROM @delete_sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Alternative approach: Delete posts one by one if the above doesn't work
-- You can also temporarily disable safe update mode:
